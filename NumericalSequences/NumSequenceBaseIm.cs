...
namespace NumericalSequences
{
    public abstract class NumSequenceBaseIm<T> : NumSequenceBaseC<T> where T: NumSequenceBaseIm<T>
    {
        ...
        protected abstract void ConvertPosition(int position, out int index, out int positionWord, 
                                                out int offset);
        ...
        protected virtual bool ExtensionNeeded(int index, int position, int positionWord, 
        ...
        private ulong GetPrefix(ulong word, int size)
        {
            if (size >= bitLengthWord)
                return word;
            return word & (((ulong)1 << size) - 1);
        }
        private ulong GetSuffix(ulong word, int size)
        {
            return PerformSafeShiftRight(word, bitLengthWord - size);
        }
        private void ComputeSizePrefixSuffix(int position, int offset, int size, 
                                                out int sizePrefix, out int sizeSuffix)
        {
             sizePrefix =  bitLengthWord - position * size - offset;
             sizeSuffix =  size - sizePrefix;
        }
        
        protected ulong ShiftLeftBasic(ulong word, ulong letter, int size)
        {
            return PerformSafeShiftLeft(word, size) | letter;
        }
        protected ulong ShiftLeftBasic(ulong word, ulong letter, int size, out ulong overFlow)
        {
            overFlow = GetSuffix(word, size);
            return ShiftLeftBasic(word, letter, size);
        }
        
        protected ulong ShiftRightBasic(ulong word, int size, out ulong overFlow)
        {
            overFlow = GetPrefix(word, size);
            return PerformSafeShiftRight(word, size);
        }
        protected ulong ShiftRightBasic(ulong word, int size, ulong letter, out ulong overFlow)
        {
            return (ShiftRightBasic(word, size, out overFlow) | PerformSafeShiftLeft(letter, bitLengthWord - size));
        }

        protected ulong SetLetter(ulong word, int offset, int position, ulong letter, int size, int letterSize)
        {
            ulong newWordPrefix;
            ulong newWordSuffix;
            if (letterSize == 0)
        ...
        protected void SetLetter(ulong wordPrefix, ulong wordsSuffix, int position,
                                    int offset, ulong letter, int size, int letterSize,
                                    out ulong newWordPrefix, out ulong newWordSuffix)
        {
            ComputeSizePrefixSuffix(position, offset, letterSize, out int sizePrefix, out int sizeSuffix);
        ...
        public override T SetLetter(int position, int letter)
        {
            ...
            ConvertPosition(position, out int index, out int positionWord, out int offset);
        ...
        protected ulong GetLetter(ulong word, int offset, int position, int size, int letterSize)
        {
        ...
        protected ulong GetLetter(ulong wordPrefix, ulong wordSuffix, int offset, 
                                    int position, int size, int letterSize)
        {
            ComputeSizePrefixSuffix(position, offset, letterSize, out int sizePrefix, out int sizeSuffix);
        ...
        protected ulong InsertLetter(ulong word, int offset, int position, ulong letter, int size, int letterSize)
        {    
            ulong newWordPrefix;
            ulong newWordSuffix;
        ...
        protected ulong InsertLetter(ulong word, int offset, int position, ulong letter, 
                                        int size, int letterSize, out ulong overFlow)
        {
        ...
        protected void InsertLetter(ulong wordPrefix, ulong wordSuffix, int offset,
                                    int position, ulong letter, int size, int letterSize,
                                    out ulong newWordPrefix, out ulong newWordSuffix)
        {
            ComputeSizePrefixSuffix(position, offset, letterSize, out int sizePrefix, out int sizeSuffix);
            ulong letterPrefix = letter & (((ulong)1 << sizePrefix) - 1);
        ...
        protected void InsertLetter(ulong wordPrefix, ulong wordSuffix, int offset, int position,
                                    ulong letter, int size, int letterSize, out ulong overFlow,
                                    out ulong newWordPrefix, out ulong newWordSuffix)
        {
        ...
        protected ulong DeleteLetter(ulong word, int offset, int position, int size, int letterSize)
        {
            ulong newWordPrefix;
            ulong newWordSuffix;
        ...
        protected ulong DeleteLetter(ulong word, int offset, int position, 
                                    int size, int letterSize,ulong shiftedLetter)
        {
        ...
        protected void DeleteLetter(ulong wordPrefix, ulong wordSuffix, 
                                    int offset, int position, int size, 
                                    int letterSize, out ulong newWordPrefix, out ulong newWordSuffix)
        {
            ComputeSizePrefixSuffix(position, offset,letterSize, out int sizePrefix, out int sizeSuffix);
        ...
        protected void DeleteLetter(ulong wordPrefix, ulong wordSuffix, int offset, int position,
                                    int size, int letterSize, ulong shiftedLetter, 
        ...
        protected ulong ChangeLetter(ulong word, int offset, int position, 
                                        int difference, int size, int letterSize)
        {
            if (letterSize == 0)
                return word;
        ...
        public override string ToString()
        {
            ...
            ConvertPosition(0, out int index, out int positionWord, out int offset);
            ulong word;
            ulong number;
            ...
            for (int i = 0; i < Length; i++)
            {
                ...
                { 
                    int prefixSize =  bitLengthWord - (position - LetterSize);
                    number = GetSuffix(Words[index], prefixSize);
                    number = number | (GetPrefix(Words[index + 1], LetterSize - prefixSize) 
                                                                                        << prefixSize);
